...
namespace ReZero.SuperAPI
{
    public class DynamicApiManager : IDynamicApi
    {
        ...
        {
            var db = App.Db;
            var isAnyUrl = ZeroInterfaceListCacheManager.Instance.GetList().Any(it => it!.Url!.ToLower() == url.ToLower());
            return isAnyUrl;
        }
        ...
        private static async Task WriteAsyncSuccess(HttpContext context, DynamicApiHelper helper, HttpRequestMethod requestMethod)
        {
            ...
            var interInfo = ZeroInterfaceListCacheManager.Instance.GetList().Where(it => it.Url!.ToLower() == path).First();
